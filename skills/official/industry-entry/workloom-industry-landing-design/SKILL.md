...
**场景二：已有行业Bundle全盘升级**
电商行业Bundle从MVP升级到全盘覆盖，需要重新设计完整方案（如从3个Skills扩展到13个Skills）。

**场景三：跨行业抽象适配**
电商行业跑通后，需要把电商的经验抽象适配到餐饮/美业/教培，设计这些新行业的落地方案。
...
**设计步骤**：
1. **核心判断**：看透这个行业经营的本质是什么？（如电商经营的本质是"事件驱动的连续决策流"）
2. **五元事件建模**：将行业经营全盘映射到WorkLoom的五元事件模型（Who×Context×Object×Decision×RuleImpact）。识别行业的核心经营对象（如电商的28类经营对象），扩展objects.json。
3. **经营阶段生命周期**：定义三层阶段模型——门店级阶段（如爬坡/稳定/旺季/淡季/调整）、客户级阶段（如下单/履约/签收/售后/复购）、运营级阶段（如日/周/月/季）。扩展stages.json。
...
2. **业务域划分**：把行业经营划分为若干业务域（如电商的业务域：选品/刊登/定价/投放/客服/履约/财务/合规）。
...
5. **客群差异化配置**：针对目标行业的不同客群（如电商的腰部卖家/国内大卖/跨境集团），设计差异化配置（通过客户级Patch实现）。
...
4. **客群价值差异**：针对不同客群，测算价值差异（如电商的三类客群价值不同）。
...
2. **跨行业适配预览**：预览适配到下一个目标行业的映射关系（如电商→餐饮的适配表）。
...
| 已有行业Bundle参考 | 已跑通的行业Bundle（如电商Bundle）作为参考 | WorkLoom代码仓库 |
...
2. **替换行业词汇**：把"电商"替换为目标行业词汇
3. **保持六模块框架不变**：六模块设计框架完全复用
4. **行业特定调整**：根据行业特点，调整某些模块的设计重点（如餐饮的"系统对接"重点是POS和外卖平台，电商的"系统对接"重点是平台API和ERP）
5. **复用已沉淀的资产**：从已跑通的行业（如电商）复用模型资产、机制资产、方法论资产

### 7.3 已验证的行业

- ✅ 电商行业（WorkLoom 熊猫电商运营系统落地方案，已验证框架可行性）
- 🔜 餐饮行业（待技能一和技能二完成后调用）
- 🔜 美业行业（待技能一和技能二完成后调用）
- 🔜 教培行业（待技能一和技能二完成后调用）
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v1.0 | 2026-08-20 | 初始版本，基于WorkLoom熊猫电商运营系统落地方案设计实践抽象 |
...
| P0 | `bundles/ecommerce/bundle.json`（或目标行业bundle） | 该Bundle提供的资产清单（presets/fences/skills/schemas/ui）与版本号 |
...
**电商行业Bundle现状（2026-08-27代码快照，`bundles/ecommerce` v1.0.0）**：
- 81个Agent preset（1+2+N：集团指挥层4 + 共享中台27 + 国内军26 + 跨境军24，另继承 inspection-agent / desktop-agent）
- 基线围栏R1–R30（default_level=review，deny 优先）：R1单品单日降价≤10% auto；R2毛利红线熔断 block；R3广告ACoS连续爆表降预算请示；R11刷单/删差评/侵权词 block
- 160个官方Skill（含★门道固化48个，覆盖14个业务域）
- 经营对象28类：sku/spu/listing/price/stock/order/aftersale/review/ad_campaign/platform/shop/settlement/warehouse/supplier/member/competitor/staff/task 等
- 经营阶段8个：launch/growth/burst/stable/decline/clearance/promo_prep/promo_peak
- 一店一档Schema（14店铺×30+字段组）+ UI用例（cases.json）
