# PREVIEW_CHECKLIST · 三端预览验收清单（熊猫电商运营系统）

> 由 `pnpm preview:all` 一键拉起。验收口径：任意开发者克隆仓库后，三端可访问、数据饱满、交互无报错。
> 运行态：「熊猫优选集团」30 天经营态快照（14 店铺 / 13 平台 / 8,600+ SKU / 日均 9.6 万单）。
...
| 验证点 | 预期 |
|---|---|
| 首页经营剧场 | 糖果色主题、「熊猫运营中心」四层职场（国内战区/跨境战区/中台楼层/客服中心）全员就位、"全模拟运行态"横幅常驻 |
| 晨报卡片 | 【晨报·集团CEO】含事件库规模、14 店铺 / 13 平台动态计数 |
| 顶部导航 | 工作台 / 董事长视图可切换，URL 随路由变化（/p0、/p21 等） |
| 待审批区 | 存在待拍板事项（如 ACoS 熔断降预算 / 紧急采购请示），✓批准/✕驳回可点击 |
| 底部输入 | "像跟集团CEO说话一样输入…" ASK 入口可输入发送 |
...
## 3. C 端移动 · 买家服务前台（http://localhost:3002）

| 验证点 | 预期 |
|---|---|
| 对话页 | 「熊猫优选·买家服务前台」在线、欢迎语、快捷入口（查订单/售后退换/物流催办/常见问题） |
| 多模态会话 | 演示会话含截图（订单异常/报错页）、图片（找同款/辨型号）、视频（安装/故障）样例，AI 有确定性应答 |
| 多语言 | 8 语种应答能力可见（中/英/西/日/德/法/泰/越） |
...
- [ ] 种子注入回报计数正常（81 名数字员工 / 围栏 R1–R30 / 对象 / 事件 / 知识库 385 问）
- [ ] 熊猫优选集团运行态勾稽：广告-订单-库存-账单可对平（含约 0.3% 演示差异）
