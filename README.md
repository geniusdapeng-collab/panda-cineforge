...
电商版开箱自带完整账号体系（基座 `packages/base/accounts` 同步而来，无需配置）：

- **团队角色**：老板（owner）/运营主管（manager）/客服与运营（staff）/查看（readonly）四角色；多平台多店铺的老板登录后进**统一待办**——天猫/拼多多/抖音各店的审批与告警一屏按店分组，不切账号；
- **客服外包（伙伴域刚需）**：大促临时客服团队由老板签发**能力白名单授权**（工单处理/话术执行/报表查看），**改价、退款、广告充值等资金类能力永不对伙伴开放**；随时吊销、动作逐条可见；旺季兼职客服用**一次性工单通行证**（只看自己那张售后单）;
- **投资人/合伙人**：observer 观察者授权——只看经营看板与交付报表，30 天时效，看不了成本明细与客户数据；
- **集团卖家**：品牌公司→店铺公司两级租户 + 集团围栏下发（只紧不松）+ ERP/财务 API 密钥对接；
