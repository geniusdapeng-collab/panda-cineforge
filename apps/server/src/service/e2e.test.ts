...
async function resolveWorkspaceSlug(): Promise<string> {
  ...
  try {
    ...
    const r = await pool.query(`SELECT slug FROM workspaces ORDER BY (slug='panda-group') DESC, created_at LIMIT 1`);
    return String(r.rows[0]?.slug ?? "panda-group");
...
describe("G B 端 · 登录与守卫", () => {
  it("loginAs 返回 token + identity（owner / plan pro）", async () => {
    const r = await trpc("auth.loginAs", { input: { workspaceSlug: "panda-group", memberNo: "MEM-001" }, method: "mutation" });
    expect(r.error).toBeNull();
  ...
  it("loginAs 不存在成员 → NOT_FOUND", async () => {
    const r = await trpc("auth.loginAs", { input: { workspaceSlug: "panda-group", memberNo: "MEM-999" }, method: "mutation" });
    expect(r.error!.data?.code).toBe("NOT_FOUND");
  });
