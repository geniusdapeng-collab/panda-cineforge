/**
 * P1 晨报·经营主页（F3：真实 API 接线版；PRD P1-①②③ 逐条对账）
 *  - 左栏 ConversationList：📌 置顶（夜班中心频道/昨夜日报）+ 待办（审批请求 badge）+ 任务线程（状态点实时）+ 问答
 *  - 中栏 MessageFlow：系统分隔线 → 交接班卡（P1E3，三计数与 P3 强一致 F4.4）→ KPI 投影（店铺档案 history_curve 真实数据）
 *    → 巡检雷达推送（P1E4，一键派单接 inspection.dispatch；无异常显「昨夜一切正常」）
 *  - 右栏：档案 chips / 夜班班组状态卡 / 在线成员人机混编（P1E6）/ 平台巡检状态
 *  - 底部：航线设定台（P1E1，Enter/启航→threads.dispatch；含糊→反问不建任务 F3.2）+ 快捷目标（P1E7，F3.5 电商 6 条）
...
interface ArchiveShape {
  property?: { name: string; city: string; shops?: number; platforms?: number; segment?: string };
  history_curve?: Record<string, { gmv_cny: number; margin_rate: number; acos: number; turnover_days: number }>;
}
interface ProfileResp { archive: ArchiveShape; stage: string | null; name: string }

/** 电商 6 快捷目标（F3.5：ACoS熔断复盘/断货预警/差评响应/调价建议/对账说明/大促战备；行业 Bundle 预置） */
const QUICK_GOALS = [
  { label: "调价建议", text: "给出 SKU-3C-1001 磁吸充电宝在天猫/拼多多的调价建议", preset: "competitor-radar" },
  { label: "ACoS 熔断复盘", text: "复盘昨夜亚马逊 ACoS 爆表与预算熔断动作", preset: "amz-ppc" },
  { label: "断货预警", text: "汇总可售天数 <7 天的 SKU 与补货建议", preset: "demand-planner" },
  { label: "差评响应", text: "起草最新差评的回复（2h SLA）", preset: "service-qc" },
  { label: "对账说明", text: "昨夜 13 平台对账差异说明", preset: "multi-reconciler" },
  { label: "大促战备", text: "双11 大促战备清单与备货沙盘", preset: "biz-analyst" },
];
...
export default function P1() {
  ...
  // KPI 投影（店铺档案 history_curve 真实数据；最新月 vs 上月；截至=档案口径月末）
  const kpis = useMemo(() => {
    ...
    return [
      { name: "月 GMV", value: `¥${(cur.gmv_cny / 1e8).toFixed(1)}亿`, delta: pct(cur.gmv_cny, prev?.gmv_cny) },
      { name: "毛利率", value: `${(cur.margin_rate * 100).toFixed(1)}%`, delta: pct(cur.margin_rate, prev?.margin_rate) },
      { name: "ACoS", value: `${(cur.acos * 100).toFixed(0)}%`, delta: pct(cur.acos, prev?.acos) },
        ...
        {isLoading ? (
          ...
          <div className="flex-1 space-y-3.5">
            <SystemDivider
              ...
              summary={`熊猫优选集团 · ${me?.identity.name ?? ""} 已上线（演示身份 MEM-001）`}
            />
            ...
            {/* KPI 全息仪表（店铺档案 history_curve 投影；截至时间必显 §5.7） */}
