...
export default function Onboarding() {
  ...
  const [industry, setIndustry] = useState("电子商务");
  ...
  return (
    <div className="min-h-screen bg-[#07070d] px-4 py-8 text-ink">
      <div className="mx-auto max-w-2xl">
        ...
        {step === 2 && (
          <div className="space-y-4 rounded-xl border border-line bg-panel/70 p-5">
            <div className="text-sm font-bold">经营主体 <span className="text-xs font-normal text-ink3">写入店铺档案，成为数字团队的上下文</span></div>
            <label className="block text-xs text-ink3">主体名称
              <input className={`${inputCls} mt-1`} value={bizName} onChange={(e) => setBizName(e.target.value)} placeholder="如：熊猫优选集团" />
            </label>
            <label className="block text-xs text-ink3">行业
              <input className={`${inputCls} mt-1`} value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder="如：电子商务 / 跨境电商 / 品牌出海" />
            </label>
            <label className="block text-xs text-ink3">经营简介（可选）
              <textarea className={`${inputCls} mt-1 h-20 resize-none`} value={note} onChange={(e) => setNote(e.target.value)} placeholder="主营类目、平台店铺数量、经营重点……数字员工会以此为背景工作" />
            </label>
            <div className="flex items-center justify-between pt-1">
                  ...
                  <button key={s.key} disabled={assembling} onClick={() => void choose(s.key)}
                    ...
                    {recommended && (
                      <div className="mt-1.5 text-[11px] leading-relaxed text-ink2">
                        先体检再托管 · 系统只读不写，不动一根手指 · 15–30 分钟出《快速体检报告》
                      </div>
                    )}
                  </button>
                );
              ...
              <div className="space-y-2 rounded-lg border border-go/50 bg-go/5 p-4">
                ...
                {assembly.stage === "audit" && (
                  <div className="rounded border border-holo/40 bg-holo/10 px-3 py-2 text-[11px] leading-relaxed text-holo">
                    体检期建议立即运行快照快扫：<code>pnpm audit:scan</code>——15–30 分钟出《快速体检报告》（一店一份 + 集团总览，异常 P0/P1/P2 分级）。
        ...
        {step === 5 && !done && (
          <div className="space-y-4 rounded-xl border border-gline bg-panel/70 p-5">
            ...
            <div className="space-y-2 rounded-lg border border-line p-3">
              ...
              <input className={inputCls} placeholder="文档标题，如《买家服务须知》" value={docTitle} onChange={(e) => setDocTitle(e.target.value)} />
...
……" value={docMd} onChange={(e) => setDocMd(e.target.value)} />
            ...
            <div className="space-y-2 rounded-lg border border-line p-3">
              ...
              <div className="flex gap-2">
                <input className={inputCls} placeholder="测试问题，如：跨境订单关税谁承担？" value={testQ} onChange={(e) => setTestQ(e.target.value)} />
