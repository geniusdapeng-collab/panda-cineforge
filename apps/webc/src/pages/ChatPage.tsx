...
import type { BusinessCard, Citation, MediaRef, MemberInfo, Order } from "../lib/types";
import { CitationCard, CatalogCard, GuideCard, MediaThumb, MemberCard, OrderCard, ProductCard, TicketNoticeCard } from "../components/cards";
    ...
    void ensureSession().then((s) => {
      ...
      if (cached.length > 0) {
        ...
        const history = (cfg.demoHistory ?? []).map((m) => ({
          id: nextId(), role: m.role, text: m.text, shown: m.text.length, ts: Date.now(),
          media: m.media, cards: m.cards, citations: m.citations,
        }));
        setMsgs([
          ...history,
                ...
                {m.role === "user" ? (
                  <div className="flex flex-col items-end">
                    <div className="flex justify-end">
                      ...
                      >
                        {m.media && <MediaThumb media={m.media} />}
                        {m.text}
                      </div>
                    </div>
                    {m.failed && (
                      ...
                      {done &&
                        m.cards?.map((c, i) =>
                          c.kind === "order" ? (
                            ...
                            <ProductCard
                              ...
                              product={
                                c.data as unknown as {
                                  name: string; spec?: string; priceYuan?: number; tag?: string; thumb?: string; note?: string;
                                }
                              }
                            />
                          ) : c.kind === "guide" ? (
                            <GuideCard
                              key={`g-${i}`}
                              guide={c.data as unknown as { title: string; steps: string[]; note?: string }}
                            />
                          ) : null,
                        )}
