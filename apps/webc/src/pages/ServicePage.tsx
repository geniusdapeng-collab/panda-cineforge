  ...
  const submit = async () => {
    ...
    try {
      const t = await api.createTicket({
        kind: entry.kind,
        title: title.trim(),
        payload: { description: desc.trim(), orderNo: orderNo.trim() },
      });
      setTicketId(t.id);
    } catch {
      setDemo(true);
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-4 py-4">
          ...
          <label className="block">
            ...
            <textarea
              ...
              rows={4}
              placeholder="补充商品、数量、问题情况等（可稍后向客服发照片/视频）"
          ...
          <label className="block">
            <span className="mb-1.5 block text-[12px] text-ink2">关联订单号</span>
            <input
              value={orderNo}
              onChange={(e) => setOrderNo(e.target.value)}
              placeholder="例如 OD-20260824-7781"
              className="h-11 w-full rounded-xl border border-line bg-bg900 px-3.5 text-[13.5px] text-ink outline-none placeholder:text-ink3 focus:border-gline"
            />
          </label>
        </div>
