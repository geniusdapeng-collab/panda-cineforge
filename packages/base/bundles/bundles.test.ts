...
describe.runIf(RUN_DB)("行业装配 PG 集成（F2.10 铁律）", async () => {
  ...
  it("校验留痕：recheck 写 bundle.check_run 事件（P7E3 留痕可查）", async () => {
    const r = await recheckBundle(app, gw, scope, "ecommerce", "MEM-001");
  ...
  it("激活幂等：ecommerce 已激活仍可重激活并留痕 bundle.activate", async () => {
    const r = await activateBundle(app, gw, scope, "ecommerce", "MEM-001");
    expect(r.eventId).toMatch(/^E-\d+$/);
    const ind = await qApp(`SELECT industry FROM workspaces WHERE id=$1`, [scope.workspaceId]);
    expect(ind.rows[0]!.industry).toBe("ecommerce");
  });
  ...
  it("H-15 第三行业五要素填充即可运行：填满五槽 → 6/6 全绿 → 激活切换 → 底座代码零改动（§2.3/§2.4）", async () => {
    ...
    try {
      ...
      await activateBundle(app, gw, scope, "ecommerce", "MEM-001"); // 还原演示工作区
    }
