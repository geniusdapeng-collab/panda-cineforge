...
d("PG 集成夜班闭环（种子库）", async () => {
  ...
  it("开启夜班：ready→running + 围栏快照 ecom-baseline/v1（F2.6）+ 留痕", async () => {
    ...
    try {
      ...
      expect(r.rows[0]).toMatchObject({ status: "running", fence_snapshot_version: "ecom-baseline/v3", candidate_count: 2 });
    } finally { c.release(); }
  });
