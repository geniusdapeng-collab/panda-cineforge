...
export function ticketKindForServiceRequest(text: string): TicketDraftKind {
  if (/修|坏|漏水|冰柜|热水|灯|水管|设备/.test(text)) return "repair";
  if (/送|拿|补货|换|加一|多要|再来/.test(text)) return "delivery";
  return "other";
}

/** biz_query → 工具调用描述（工具契约，由 server 层执行） */
export function bizToolFor(text: string, cUserId: string): ToolCallRequest {
  if (/账单|运费|发票/.test(text)) return { tool: "biz.query_bill", params: { cUserId } };
...
export async function handleMessage(
  ...
  if (intent === "kb_qa") {
    ...
    if (deps.chatLlm) {
      ...
      mock = true;
      degraded = true;
      answer = `[mock] 您好，我是熊猫优选集团智能客服。您说的「${input.text.slice(0, 20)}」我已收到，可继续咨询商品问题。`;
      confidence = 0.3;
    }
  }
