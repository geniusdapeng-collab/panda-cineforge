 ...
 *  明示报修词（修/修一下/坏了/故障/维修）直连 service_request（如「冰柜坏了帮我修一下」）。
 */
...
/** 明示报修/故障词：直连 service_request（疑问句也不拦——「冰柜怎么修」仍需上门） */
const REPAIR_KEYWORDS = ["维修", "修一下", "坏了", "故障", "报修", "漏水", "不制冷", "不运转"];

/** 规则表（complaint > biz_query > service_request > kb_qa；service_request 只收「指令型」服务词） */
const RULES = {
  complaint: ["投诉", "差评", "不满意", "破损", "质量差", "态度差", "举报", "维权", "退款理由"],
  // 价格/会员类仅在有明确「我的账户/我的订单」语境时才走业务查询，
  // 否则「面膜多少钱」「会员包邮要钱吗」应走知识库（M8 评测校准：R04/A16/A20）
  biz_query: ["我的订单", "订单", "运费", "账单", "积分", "余额", "发票记录", "售价", "我的会员", "会员卡", "我的积分", "我的余额"],
  service_request: ["送", "拿", "加一", "补货", "换货", "开发票", "续费", "多要", "再来一份", "修"],
  // 注：退款/退货/换货等「售后动作指令词」不入本表——电商口径下属指令型建单（server 层 RE_AFTERSALE_ACTION 兜底），
  // 疑问句仍由 QUESTION_MARKERS 分流 kb_qa（「退货上门取件几点来」「退货时间政策」不受影响）
  kb_qa: ["几点", "时间", "政策", "发货", "会员", "优惠", "怎么", "如何", "可以带", "收费吗", "免费吗"],
} as const;
