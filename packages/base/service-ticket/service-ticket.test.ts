...
describe("createTicket 幂等 + 全生命周期留痕", () => {
  ...
  it("created→assigned→processing→done→closed 全链路 + 时间线回放 + 五元事件", async () => {
    ...
    const { ticket } = await createTicket(db, CTX,
      { kind: "repair", title: "冰柜坏了", idempotencyKey: "msg-002" }, ACTOR, emit);
    // 默认部门路由：repair → 维修组
  ...
  it("部门路由表可注入覆盖", async () => {
    ...
    const { db, emit } = setup(now);
    const { ticket } = await createTicket(db, CTX,
      { kind: "delivery", title: "送样品", idempotencyKey: "msg-004" }, ACTOR, emit);
    const assigned = await assignTicket(db, CTX, ticket.id, ACTOR,
      { routes: { delivery: "贵宾组", repair: "维修组", complaint: "客服主管", other: "客服组" } }, emit);
    expect(assigned.dept).toBe("贵宾组");
  });
